...
class TestExampleFilesSync:
    ...
    def test_minimal_ts(self):
        ...
        assert not ts_has_error(source), "Tree-sitter found errors in minimal.tahta"

    def test_minimal_py(self):
        source = (EXAMPLES_DIR / "minimal.tahta").read_text()
        assert not py_has_error(source), "Python parser found errors in minimal.tahta"

    def test_tutorial_ts(self):
        source = (EXAMPLES_DIR / "tutorial.tahta").read_text()
        assert not ts_has_error(source), "Tree-sitter found errors in tutorial.tahta"

    def test_tutorial_py(self):
        source = (EXAMPLES_DIR / "tutorial.tahta").read_text()
        assert not py_has_error(source), "Python parser found errors in tutorial.tahta"
...
class TestPropertySync:
    @pytest.mark.parametrize("entity,prop", [
...
class TestConditionOperatorSync:
    ...
    def test_condition_operators(self, op):
        ...
        assert not py_has_error(source), f"Python parser rejected operator: {op}"

    @pytest.mark.parametrize("op", ["<", ">", "=", "<=", ">="])
    def test_weight_condition_operators(self, op):
        source = f"C (card:c)\n\tweight: 2.0 when counter:x {op} 50\n"
        assert not ts_has_error(source), f"Tree-sitter rejected weight condition operator: {op}"
        assert not py_has_error(source), f"Python parser rejected weight condition operator: {op}"
...
class TestCommandSync:
    ...
    def test_commands(self, cmd):
        ...
        assert not py_has_error(source), f"Python parser rejected command: {cmd}"
