...
class Lexer:
    ...
    # Patterns
    # Entity header: Name (type, id, ...flags) - e.g., "Hazine (counter, hazine, killer)"
    ...
    CHOICE_EMPTY_LABEL_PATTERN = re.compile(r'^\*\s*:\s*(.*)$')  # * : commands (blind choice)
    ...
    def __init__(self, source: str, filename: str = "<string>"):
        ...
        self.pos = 0
        self.indent_char: Optional[str] = None  # '\t' or ' ', detected from first indented line
    ...
    def _check_indent_consistency(self, raw: str, line_number: int):
        """
        Check that indentation is consistent (all tabs or all spaces, not mixed).
    ...
    def _classify_line(self, raw: str, line_number: int) -> Line:
        ...
        if indent == 0 and stripped.startswith('import '):
            ...
            if (rest.startswith('"') and rest.endswith('"')) or \
               (rest.startswith("'") and rest.endswith("'")):
                ...
                return Line(
                    LineType.IMPORT, raw, line_number, indent,
        ...
        # Choice with empty label (blind choice): * : commands - MUST be indented
        ...
        if indent > 0:
            return Line(
                LineType.INDENTED, raw, line_number, indent,
                value=stripped
            )
