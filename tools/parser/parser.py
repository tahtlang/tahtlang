...
from .lexer import EntityType, Lexer, Line, LineType, Modifier
...
class Parser:
    ...
    def _parse_import(self, line: Line):
        ...
        import_node = Import(
            path=line.import_path or "", loc=self._make_loc(line)
        )
    ...
    def _make_loc(self, line: Line) -> SourceLocation:
        ...
        return SourceLocation(
            file=self.current_file, line=line.line_number, column=line.indent
        )
    ...
    def _parse_settings(self, name: str, entity_id: str, header: Line):
        ...
        if "starting_flags" in props:
            starting_flags = tuple(
                self._parse_flag_list(props["starting_flags"])
            )
        ...
        self._settings = Settings(
            ...
            game_over_on_zero=self._parse_bool(
                props.get("game_over_on_zero", "")
            ),
            game_over_on_max=self._parse_bool(
                props.get("game_over_on_max", "")
            ),
    ...
    def _parse_counter(
        self, name: str, entity_id: str, modifiers: set[Modifier], header: Line
    ):
        ...
        Virtual counter (aggregate):
            Overall (counter:overall)
                source: [counter:spiritual, counter:military,
                         counter:demography, counter:treasure]
                aggregate: average
        ...
        props = self._collect_properties(
            primary_key="start", valid_keys=self.COUNTER_KEYS
        )
        ...
        aggregate = self.AGGREGATE_MAP.get(
            props.get("aggregate", "").lower().strip()
        )
    ...
    def _parse_lockturn(self, value: str, line: Line) -> Lockturn:
        ...
        try:
            ...
            raise self._error(
                f"Invalid lockturn value: '{value}' "
                "(must be integer, 'once', or 'dispose')",
                line,
            )
    ...
    def _parse_bearer(self, value: str, line: Line) -> Bearer:
        ...
        char_id = self._strip_type_prefix(
            value[:paren_start].strip(), "character"
        )
        variant_id = self._strip_type_prefix(
            value[paren_start + 1 : -1].strip(), "variant"
        )
    ...
    def _parse_commands(self, commands_str: str, line: Line) -> list[Command]:
        ...
        Example: '+counter:hazine 20, -counter:halk 10, '
                 '+flag:winter, card:next-event'
    ...
    def _parse_single_command(
        ...
        if part_lower.startswith("-flag:"):
            return FlagClear(
                flag_id=part[6:].strip(), loc=self._make_loc(line)
            )
    ...
    def _parse_branch_cmd(self, part: str, line: Line) -> Optional[CardBranch]:
        ...
        if card_ids:
            return CardBranch(
                card_ids=tuple(card_ids), loc=self._make_loc(line)
            )
        return None
    ...
    def _parse_counter_cmd(
        self, rest: str, line: Line
    ) -> Optional[CounterMod]:
        ...
        if not value:
            return None
        return CounterMod(
            counter_id=counter_id, value=value, loc=self._make_loc(line)
        )
    ...
    def _parse_value_or_range(
        self, s: str, line: Line
    ) -> Optional[ValueOrRange]:
    ...
    def _parse_condition_list(self, value: str, line: Line) -> list[Condition]:
        ...
        - '!flag:fortification'   -> [FlagCondition('fortification',
                                                    negated=True)]
    ...
    def _try_parse_counter_condition(
        ...
        """Try to parse 'counter:id OP value'.
        Returns None if not a counter condition.
